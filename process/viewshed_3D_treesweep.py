...
def sweep_from_tree(loc_dsm, tree_r, tree_c, tree_z, res,
                     query_rows, query_cols, query_zs, viewshed_range_m):
    """
    以一棵树(tree_r, tree_c, tree_z)为origin做sweep。
    loc_dsm里的每个像素都是"遮挡物"(用它自己的DSM高度)。
    query_rows/cols/zs是一批"只负责被查询、不参与遮挡"的观察点(building-floor)。
    返回一个跟query点等长的bool数组, True=这个观察点能看到这棵树。
    """
    ...
    # 遮挡物: 仰角 = (DSM高度 - 树的高度) / 距离, 反正切
    # 注意方向: 这里算的是"从树看向这个像素"的仰角, 越高代表越往上遮挡
    blocker_elev_angle = np.arctan2(loc_dsm - tree_z, np.maximum(dist_m_grid, 1e-9))
    ...
    b_elev = blocker_elev_angle[in_range]
    b_bin = bin_grid[in_range]
    b_is_query = np.zeros(len(b_dist), dtype=bool)
    b_query_idx = np.full(len(b_dist), -1, dtype=int)

    # query点: 同样算它们相对树的距离/角度/仰角, 但标记为"only被比较, 不参与遮挡刷新"
    qdr = query_rows - tree_r
    qdc = query_cols - tree_c
    q_dist = np.sqrt(qdr ** 2 + qdc ** 2) * res
    q_valid = (q_dist > 0) & (q_dist <= viewshed_range_m)

    q_elev = np.arctan2(query_zs - tree_z, np.maximum(q_dist, 1e-9))
    ...
    q_orig_idx = np.where(q_valid)[0]  # 记录这些query点在原始数组里的下标, 方便结果对应回去

    # 合并遮挡物和query点, 一起按(bin, distance)排序
...
def calculate_green_view_tree_sweep(
        dsm_path: Path,
        dem_path: Path,
        tree_crown_shp: Path,
        building_shp: Path,
    ...
    print("4. 从每棵树的树冠顶出发, 用sweep算法判断可见性...")
    ...
    for t_idx, t_row in tqdm(trees.iterrows(), total=len(trees), desc="Tree Progress"):
        ...
        if not (0 <= row_t < dsm_array.shape[0] and 0 <= col_t < dsm_array.shape[1]):
            continue
        tree_z = dsm_array[row_t, col_t]  # 用树冠顶的DSM高度当这棵树的origin高度
        ...
        local_tree_r = row_t - r_min
        local_tree_c = col_t - c_min

        local_q_rows = obs_rows[nearby_idx] - r_min
        local_q_cols = obs_cols[nearby_idx] - c_min
        local_q_zs = obs_zs[nearby_idx]

        visible_flags = sweep_from_tree(
            loc_dsm, local_tree_r, local_tree_c, tree_z, res,
            local_q_rows, local_q_cols, local_q_zs, viewshed_range_m
        )

        dists = np.sqrt((obs_rows[nearby_idx] - row_t) ** 2 +
...
if __name__ == "__main__":
    calculate_green_view_tree_sweep(
        ...
        output_shp=Path(r"C:\Users\xhe40\Thesis_Data\Campus\building_result_tree_sweep.shp"),
    )